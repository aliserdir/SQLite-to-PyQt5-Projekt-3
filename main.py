...
global id, vorname, nachname, stadt, telefon, email

# Kullanicilar tablosunun alanlari (id INT, vorname TEXT, nachname TEXT, stadt TEXT, telefon TEXT, email TEXT)
...
class Window(QtWidgets.QMainWindow):
    ...
    def btnKaydetClick(self): 
        ...
        vorname = self.ui.txtIsim.text()
        nachname = self.ui.txtSoyisim.text()
        stadt = self.ui.txtSehir.text()
        telefon = self.ui.txtTelefon.text()
        email = self.ui.txtEmail.text()

        try:
            self.conn = sql.connect("TelefonDaten.db")
            self.c = self.conn.cursor() 
            self.c.execute("INSERT INTO Personen VALUES (?,?,?,?,?,?)",(id, vorname, nachname, stadt, telefon,email))
            self.conn.commit()
            self.c.close()
            self.conn.close()
    ...
    def btnListeleClick(self):  
        ...
        self.ui.tblListele.setHorizontalHeaderLabels(('ID','Vorname','Nachname','Stadt','Telefon','Email'))
    ...
    def btnGuncelleClick(self):  
        ...
        try:
            self.conn = sql.connect("TelefonDaten.db")
            self.c = self.conn.cursor()  
            self.c.execute("UPDATE Personen SET vorname = ?, nachname = ?, stadt = ?, \
                telefon = ?, email = ? WHERE id = ?",(vorname, nachname, stadt, telefon, email, id))
            self.conn.commit()
            self.c.close()
            self.conn.close()
    ...
    def btnSilClick(self): 
        ...
        try:
            self.conn = sql.connect("TelefonDaten.db")
            self.c = self.conn.cursor() 
            self.c.execute('DELETE FROM Personen WHERE id = ?  ', (id,))
            self.conn.commit()
            self.c.close()
            self.conn.close()
